...
_SYSTEM_PROMPT = (
    "You are an exam question analyst. "
    ...
    "then briefly why each wrong option is incorrect (name each option explicitly),\n"
...
def _format_question(q: dict) -> dict:
    """Slim down to only what the LLM needs."""
...
def enrich_questions(questions: list[dict], _retry: int = 0) -> dict[str, dict]:
    """
    Send a batch of questions to Groq in a single API call.
    Returns {str(id): result_dict} for each question.
    On unrecoverable failure returns {} so the caller marks all as failed.
    """
    ...
    try:
        ...
        if _retry < 3:
            ...
            log.warning(f"Rate limit — retrying in {wait}s (attempt {_retry + 1}/3)")
            time.sleep(wait)
            return enrich_questions(questions, _retry + 1)
        log.error(f"Rate limit exceeded after 3 retries for ids={[q.get('id') for q in questions]}")
        return {}

    except Exception as e:
        log.error(f"Groq error (ids={[q.get('id') for q in questions]}): {e}")
        return {}
